...
- 機能4：編成検索
  ...
  - 多少工夫はしましたが基本総当たりなのでクソ重です
    - 編成検索ボタンが灰色のうちは計算中です
    - 開発者のPCでは1分くらいかかります
  - あまりにも重いのでLR以上の社員のみを候補にしています
    - 実用的な社員だとワンダーギャルとかテングガールとかが対象外
    - そのうち改善予定
...
- 環境によってはウィルス対策ソフトが文句言ってくるかも
- もろもろの計算式は想像で補っている上、演算誤差を考慮せずに組んでいるので多少誤差は出ます
  - 特に低レベル帯では顕著です
  - なので低レベル帯はこんなの使わずレベリングして押しつぶすのが吉
- 敵のときと味方の時でステータスが違う社員がいますが、このシミュでは対応できていません
  - 敵側でも味方時のステータスで動作します
  - そのうち改善予定
- まだ試作しただけなので変な入力をしたら落ちるし多分バグも残ってます
